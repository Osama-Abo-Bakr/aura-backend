...
def _make_auth_override(user_dict: dict | None = None):
    ...
    if user_dict is None:
        user_dict = {"sub": "00000000-0000-0000-0000-000000000001", "email": "test@example.com", "role": "authenticated"}
...
class TestMeNullProfile(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.user = {"sub": "00000000-0000-0000-0000-000000000001", "email": "test@example.com", "role": "authenticated"}
    ...
    def test_me_returns_null_profile_when_no_profile(self, mock_admin: MagicMock) -> None:
        ...
        data = response.json()
        self.assertIsNone(data["profile"], "Expected profile to be null when no profile exists")
...
class TestUpsertProfileDatetimeSerialization(unittest.TestCase):
    ...
    def test_upsert_profile_serializes_datetime(self, mock_admin: MagicMock) -> None:
        ...
        mock_resp = MagicMock()
        mock_resp.data = [profile_data]
        mock_admin.table.return_value.upsert.return_value.execute.return_value = mock_resp
        ...
        self.assertIsInstance(upsert_data.get("date_of_birth"), str, "date_of_birth should be a string, not a datetime object")
...
class TestSignoutBearerToken(unittest.TestCase):
    ...
    def test_signout_passes_bearer_token(self, MockAuthService: MagicMock) -> None:
        ...
        response = self.client.post("/api/v1/auth/signout", headers={"Authorization": "Bearer my-test-token"})
    ...
    @patch("app.api.v1.auth.AuthService")
    def test_signout_does_not_pass_empty_token(self, MockAuthService: MagicMock) -> None:
        """POST /auth/signout should never pass an empty Bearer token."""
        svc = MockAuthService.return_value
        svc.signout.return_value = None

        response = self.client.post("/api/v1/auth/signout", headers={"Authorization": "Bearer my-valid-token"})

        self.assertEqual(response.status_code, 200)
        call_args = svc.signout.call_args
        token_passed = call_args[0][0]
        self.assertNotEqual(token_passed, "", "signout should not receive an empty string")
        self.assertTrue(token_passed.startswith("my-"), f"Expected raw token, got: {token_passed}")
